...
; ------------------------------------------------------------------- ;
; Driver for the External PS/2 Mouse, that connects to the PIO Port A ;
; ------------------------------------------------------------------- ;
...
extmouse_init:
    ; Set user port as input
    ld a, IO_PIO_INPUT
    out (IO_PIO_USER_CTRL), a
    ; Set interrupt vector to 4 (interrupt_portA_handler)
    ld a, 4
    out (IO_PIO_USER_CTRL), a
    ...
    PUBLIC interrupt_pio_user_handler
interrupt_pio_user_handler:
    ex af, af'
    exx
    ; Received byte in B
    in a, (IO_PIO_USER_DATA)
...
NEW_DRIVER_STRUCT("MOUS", \
                  extmouse_init, \
                  extmouse_read, extmouse_write, \
                  extmouse_open, extmouse_close, \
                  extmouse_seek, extmouse_ioctl, \
                  extmouse_deinit)