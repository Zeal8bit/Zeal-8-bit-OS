...
/* Raw keyboard example: switch the keyboard to raw mode, echo every pressed key
 * and quit when Enter is pressed. In raw mode both key presses and releases are
 * sent to the program, values >= 0x80 mark special keys or release events and
 * are ignored here. See zos_keyboard.h for the key codes and modes. */
int main(void) {
    char key;
    uint16_t size;
    zos_err_t err;
