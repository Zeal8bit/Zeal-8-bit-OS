...
_start:
    ; Put the keyboard in RAW mode: every key press and key release is sent to us
    ; when reading. Values >= 0x80 are special keys or release events, let's
    ; ignore them in this example.
    ld h, DEV_STDIN
...
_loop:
    ; Read a single byte from the keyboard, blocking until a key is pressed.
    ld h, DEV_STDIN
    ld de, _key
    ld bc, 1
    READ()
