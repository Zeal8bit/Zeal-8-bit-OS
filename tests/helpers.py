...
Each example declares its expectations in a small pytest_<name>.py file, the
actual build/run logic lives here so we don't duplicate it everywhere.
"""
...
# Binary that must be present in PATH to build an example, per toolchain
TOOLCHAIN_TOOL = {
    "gnu-as": "z80-elf-as",
    "sdcc": "sdcc",
...
def find_binaries(example_dir):
    """Raw binaries produced by the example, in the root dir, bin/ or build/.

    CMake's internal configure artifacts (build/CMakeFiles/...) are ignored,
...
class ZealEmulator:
    """One built example running on the emulator (the device under test).

    The example must be built first (see tests/build_all.py). The emulator
    binary comes from the ZOS_EMULATOR environment variable and the OS image
    (passed via --rom) comes from ZOS_IMG.

    The emulator is spawned in console mode, and in debug mode when requested.
    In debug mode it reads control commands from stdin and prints a `debug> `
    prompt. On startup the machine is run up to a breakpoint at 0x4000 (the
    program entry point) and then handed over to the test:
    ...
    def run_tstates(self, tstates, timeout=10):
        """Advance the emulation by an exact number of Z80 T-states, waiting
